function [outIm,whatScale,Direction] = FrangiFilter2D(img, options)
...
ALLfiltered=zeros([size(img) length(sigmas)]);
ALLangles=zeros([size(img) length(sigmas)]);
...
for i = 1:length(sigmas)
    ...
    Ifiltered = exp(-Rb/beta) .*(ones(size(img))-exp(-S2/c));
...
if length(sigmas) > 1
    ...
    outIm = reshape(outIm,size(img));
    if(nargout>1)
        whatScale = reshape(whatScale,size(img));
    end
    if(nargout>2)
        Direction = reshape(ALLangles((1:numel(img))'+(whatScale(:)-1)*numel(img)),size(img));
    end
else
    outIm = reshape(ALLfiltered,size(img));
    if(nargout>1)
            whatScale = ones(size(img));
    end
    if(nargout>2)
        Direction = reshape(ALLangles,size(img));
    end
end