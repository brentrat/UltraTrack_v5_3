...
% Last Modified by GUIDE v2.5 02-Aug-2024 09:24:40
...
% Initialise default block sizes
handles.BlockSize = [21 71];
...
if ismac || isunix
    save([directory '/ultrasound_tracking_settings.mat'], 'ImageDepth',...
        'Position', 'Default_Directory','BlockSize');
end
...
handles.ImStack = autocrop(handles.ImStack);
handles.vidHeight = size(handles.ImStack,1);
handles.vidWidth = size(handles.ImStack,2);
...
if isfield(handles,'Region')
    ...
    for i = 1:length(handles.Region)
        ...
        if isfield(handles,'ImStack') && isfield(handles.Region(i),'fas_length')
            ...
            nz = handles.Region(i).fas_length(:,1) ~= false;
...
if isfield(handles,'movObj')||isfield(handles,'BIm')||isfield(handles,'ImStack')
    ...
    for i = 1:length(handles.Region)
        for k = 1:length(handles.Region(i).Fascicle)
            ...
            if isfield(handles.Region(i).Fascicle(k),'fas_x_corr') && ~isempty(handles.Region(i).Fascicle(k).fas_x_corr)
...
if isfield(handles,'ImStack')
    [fileout, pathout, FI] = uiputfile('*.png', 'Save video as');

    if FI > 0
        IM_out = getframe(handles.axes1);
        imwrite(IM_out.cdata,[pathout fileout], 'PNG');
    end
    ...
    for i = 1:length(handles.Region)
        ...
        if isfield(handles,'movObj')||isfield(handles,'BIm')||isfield(handles,'ImStack') && isfield(handles.Region(i),'fas_length')
            ...
            if isfield(handles.Region(i),'fas_length_corr') && ~isempty(handles.Region(i).fas_length_corr)
                ...
                %                 Fdat.Region(i).points = handles.Region(i).points;
            end
        end
    end
end
                ...
                if ~isempty(handles.Region(i).fas_length)
                    ...
                    for j = 1:length(handles.Region(i).Fascicle)
                        ...
                        if isfield(handles.Region(i).Fascicle(j),'fas_x_corr') && ~isempty(handles.Region(i).Fascicle(j).fas_x_corr)
                            nz = handles.Region(i).fas_length_corr(:,j) ~= false;
                            time = (dt:dt:length(handles.Region(i).fas_length_corr(:,j))*dt)+((handles.start_frame-1)*dt);

                            % Draws 1 fascicle for 1 region
                            if length(find(nz==1)) > 1
                                y_min(i,j) = min(handles.Region(i).fas_length_corr(nz,j));
                                y_max(i,j) = max(handles.Region(i).fas_length_corr(nz,j));
                                hold on; plot(time(nz),handles.Region(i).fas_length_corr(nz,j))
                            else
                                tPoint = find(nz==1);
                                y_min(i,j) = handles.Region(i).fas_length_corr(tPoint);
                                y_max(i,j) = handles.Region(i).fas_length_corr(tPoint);
                                hold on; plot(time(tPoint),handles.Region(i).fas_length_corr(tPoint)); hold off;
                            end

                            % plot the fascicle length on the graph relative to time
                            if isfield(handles.Region(i),'fas_length_keyframe')
                                dt = 1/handles.FrameRate;
                                nnz = handles.Region(i).fas_length_keyframe(:,j) ~= false;
                                ...
                                hold on; plot(time(nz),handles.Region(i).fas_length_corr(nz,j),time_keyframe(nnz),FL_keyframe,'ro');
                            else
                                hold on; plot(time(nz),handles.Region(i).fas_length_corr(nz,j))
                            end
                            ...
                            nz = handles.Region(i).fas_length(:,j) ~= false;
                            time = (dt:dt:length(handles.Region(i).fas_length(:,j))*dt)+((handles.start_frame-1)*dt);
                            FL = handles.Region(i).fas_length(nz,j);
                            %PEN = handles.Region(i).fas_pen(nz,j);

                            % Draws 1 fascicle for 1 region
                            if length(find(nz==1)) > 1
                                y_min(i,j) = min(FL);
                                y_max(i,j) = max(FL);
                                hold on; plot(time(nz),FL)
                            else
                                tPoint = find(nz==1);
                                y_min(i,j) = FL;
                                y_max(i,j) = FL;
                                hold on; plot(time(tPoint),FL); hold off;
                            end

                            if isfield(handles.Region(i),'adjusted_flength')&& size(handles.Region(i).adjusted_flength,1) > 1
                                Adj_FL = handles.Region(i).adjusted_flength(nz,j);
                                hold on; plot(time(nz),Adj_FL)
                            end

                            %                             if size(FL,1)>=frame_no
                            %                                 text(handles.Time(frame_no+handles.start_frame-1)-0.2,FL(frame_no,1)...
                            %                                     ,num2str(round(FL(frame_no,1))),...
                            %                                     'FontSize',10,'FontWeight','bold');
                            %                             end
...
handles.ln = plot(handles.length_plot,[handles.Time(frame_no+handles.start_frame-1) handles.Time(frame_no+handles.start_frame-1)],[L_range(1) L_range(2)],'w');
set(handles.length_plot,'XLim',[0 handles.Time(end)]);
L_range = [min(min(y_min))*0.85 max(max(y_max))*1.15]; %update y-range
set(handles.length_plot,'YLim',[L_range(1) L_range(2)]);
...
if isfield(handles,'D')
    ...
    hold on
    plot([handles.Time(frame_no+handles.start_frame-1) handles.Time(frame_no+handles.start_frame-1)],[y_range(1) y_range(2)],'w');
    hold off
end
...
axes(handles.length_plot); hold on
if isfield(handles,'txt')
    delete(handles.txt);
end
if isfield(handles,'ln')
    delete(handles.ln);
end
...
L_range = get(handles.length_plot,'YLim');
handles.ln = plot([handles.Time(frame_no) handles.Time(frame_no)],[L_range(1) L_range(2)],'k');
set(handles.length_plot,'XLim',[0 handles.Time(end)]);
set(handles.length_plot,'YLim',[L_range(1) L_range(2)]);

axes(handles.mat_plot); hold on
...
y_range = get(handles.mat_plot,'YLim');
plot([handles.Time(frame_no) handles.Time(frame_no)],[y_range(1) y_range(2)],'k');
set(handles.mat_plot,'XLim',[0 handles.Time(end)]);
...
if isfield(handles,'ImStack')
    ...
    %     end
    %
    %     if isfield(handles,'Region') && isfield(handles.Region,'Fascicle')
        ...
        for i = 1:length(handles.Region)
            ...
            for f = frame_no+1:get(handles.frame_slider,'Max')
                ...
                for j = 1:length(handles.Region(i).Fascicle)
                    ...
                    %                     handles.Region(i).points{f} = points;
...
% update the image axes using show_data function
show_data(hObject, handles);
...
if isfield(handles,'ImStack')
    ...
    if ii == 1 || 2 || 3
        ...
        polipo = drawpolygon('Color',[1 1 0], 'InteractionsAllowed','all', 'Position',fasROI.position);

        % Draw aponeurosis lines
        apo_manual_up = drawline('Color',[0 0 1] , 'InteractionsAllowed','all', 'Position', apo1.line);
        apo_manual_low = drawline('Color',[0 0 1] , 'InteractionsAllowed','all', 'Position', apo2.line);
...
function ImCropped = autocrop(Im)
